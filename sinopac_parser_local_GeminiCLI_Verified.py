...
def main():
    ...
    for index, row in combined_df.iterrows():
        ...
        match = re.search(r"(\d{2}/\d{2})\s+(\d{2}/\d{2})\s+(?:\d{4}(?:\.0)?\s+)?(.*?)\s*(-?[\d,]+(?:\.0)?)$", row_text)
        ...
        if match:
            ...
            # Sanity check: If the description contains header-like keywords, it's likely a junk row.
            header_keywords = ['帳單說明', '臺幣金額', '總費用', '消費日', '卡號']
            if any(keyword in full_description for keyword in header_keywords):
                # This is a junk row, discard it and reset the buffer.
                description_buffer = []
